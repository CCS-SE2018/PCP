...
addUser = _ => {
    ...
    console.log(user);
    fetch(`http://localhost:4000/users/add?userID=${userID}&userName=${user.userName}&userPassword=${user.userPassword}&firstName=${user.firstName}&lastName=${user.lastName} `)
    .then(response => response.json())
      ...
      <div className="App">
        ...
        <p> Username</p>
          ...
          <input
              ...
              type = "password"
              onChange={e => this.setState({ user: { ...user, userPassword: e.target.value}})}
          />
          <br />
          <p>First Name </p>
          <input
